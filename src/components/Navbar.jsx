        ...
        <nav className="hidden md:flex gap-7 items-center">
            ...
            <div className="relative">
              ...
              <div className={`${profileOpen ? 'block' : 'hidden'} absolute right-0 mt-2 w-44 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-md shadow-lg py-1 z-50`}>
                <Link to="/login" className="block px-4 py-2 text-gray-800 hover:bg-orange-50 hover:text-orange-700 rounded transition-colors cursor-pointer" onClick={() => setProfileOpen(false)}>Login</Link>
                <Link to="/signup" className="block px-4 py-2 text-gray-800 hover:bg-orange-50 hover:text-orange-700 rounded transition-colors cursor-pointer" onClick={() => setProfileOpen(false)}>Sign up</Link>
              </div>
            </div>
            ...
            <div>
              ...
              {profileOpen && (
                <div className="flex flex-col gap-2 md:gap-0 pl-2 mt-3">
                  <Link to="/login" className="hover:text-orange-600 py-1" onClick={() => { setIsOpen(false); setProfileOpen(false); }}>Login</Link>
                  <Link to="/signup" className="hover:text-orange-600 py-1" onClick={() => { setIsOpen(false); setProfileOpen(false); }}>Sign up</Link>
                </div>
              )}
            <div className="flex items-center gap-3 mt-3 pl-2">
              <img src={logo} alt="User avatar" className="w-9 h-9 rounded-full object-cover border border-gray-200" />
